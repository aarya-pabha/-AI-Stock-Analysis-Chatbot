...
async def run_backtest_ui(ticker: str, start_date: str, end_date: str):
    ...
    # engine.run() is now an async generator
    async for trade_result in engine.run():
        ...
        status = f"""
        ### 🔄 Backtest in Progress... ({len(all_results)} trades found)
        - **Current Date:** {trade_result['date']}
    ...
    if not all_results:
        yield "Simulation complete. No 'Buy' signals were generated by the council during this period.", None
    else:
        df = pd.DataFrame(all_results)
        wins = len(df[df['pnl_pct'] > 0])
        ...
        - **Simulation Window:** {start_date} to {end_date}
        """
